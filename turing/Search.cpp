    ...
    for (auto j = 0; j < max + nMpm; ++j)
    {
        ...
        *activeCandidate = challenger;

        {
            ...
            const Lambda lambda = getReciprocalLambda(h);
...
void searchIntraChroma(H &h, const coding_quadtree &cqt)
{
    ...
    for (int n = 0; n < 5; ++n)
    {
        ...
        *activeCandidate = challenger;
        ...
        challenger->ContextsAndCost::lambdaDistortion += getReciprocalLambda(h) * ssd;
...
void searchIntraCu(H &h, CandidateStash<Sample> *&challenger, CandidateStash<Sample> *&champion)
{
    ...
    *activeCandidate = challenger;
    ...
    {
        stateEncodeSubstream->unfiltered[cIdx] = IntraReferenceSamples<Sample>();
        stateEncodeSubstream->filtered[cIdx] = IntraReferenceSamples<Sample>();
    }
    ...
    else
    {
        ...
        if (FORCE_PCM(h))
        {
            ...
            for (int cIdx = 0; cIdx < 3; ++cIdx)
            {
                ...
                auto const z = zPositionOf(rc) >> (rc.cIdx ? 2 : 0);
...
#ifdef DEBUG_PIECES
                assert(static_cast<Candidate<Sample>*>(h)->StatePieces<Sample>::z[rc.cIdx] == z);
                piece.rc = rc;
#endif
...
struct Search<coding_unit>
{
    template <class H> static void go(const coding_unit &cu, H &h)
    {
        ...
        if (h[cu_qp_delta_enabled_flag()])
        {
            ...
            int qpValue = static_cast<QpState *>(h)->getQpInternal(row, col);
            ...
            static_cast<QpState *>(h)->setQpValue(qpValue);
        }
        ...
        if (h[slice_type()] != I && !FORCE_PCM(h))
        {
            ...
            if (!(speed->doIntraInInter())) doTryIntra = false;
        ...
        template <class H> void go2(const prediction_unit &pu, H &h)
        {
            ...
            else
            {
                ...
                *static_cast<ContextsAndCost *>(h) = bestContextsAndCost;
                ...
                if (h[inter_pred_idc(pu.x0, pu.y0)] == PRED_BI)
                {
