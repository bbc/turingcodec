...
{
    template <class H> static void go(const buffering_period &fun, H &h)
    {
        {
...
template <class H> void Read<buffering_period>::go(buffering_period const &f, H &h)
{
    BufferingPeriod bufferingPeriod;
    auto h3 = h.extend(&bufferingPeriod);

    auto oldActiveSps = h[Active<Sps>()];
    auto h2 = h3.extend(static_cast<Hrd *>(0));
    ...
    EXPLICIT_INSTANTIATION(buffering_period)
#endif
